/*******************************************************************************
* File Name: PRINTER_B_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: PRINTER_B_IntClock_Start
********************************************************************************
*
* Summary:
...
void PRINTER_B_IntClock_Start(void) 
{
    ...
    PRINTER_B_IntClock_CLKEN |= PRINTER_B_IntClock_CLKEN_MASK;
	PRINTER_B_IntClock_CLKSTBY |= PRINTER_B_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PRINTER_B_IntClock_Stop
********************************************************************************
*
* Summary:
...
void PRINTER_B_IntClock_Stop(void) 
{
    ...
    PRINTER_B_IntClock_CLKEN &= (uint8)(~PRINTER_B_IntClock_CLKEN_MASK);
	PRINTER_B_IntClock_CLKSTBY &= (uint8)(~PRINTER_B_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PRINTER_B_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PRINTER_B_IntClock_CLKEN & PRINTER_B_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PRINTER_B_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PRINTER_B_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PRINTER_B_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PRINTER_B_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PRINTER_B_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PRINTER_B_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PRINTER_B_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PRINTER_B_IntClock_GetSourceRegister();
    uint16 oldDivider = PRINTER_B_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PRINTER_B_IntClock_CLKEN & PRINTER_B_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(PRINTER_B_IntClock_DIV_PTR, clkDivider);
                PRINTER_B_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                PRINTER_B_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(PRINTER_B_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PRINTER_B_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(PRINTER_B_IntClock_DIV_PTR, clkDivider);
				PRINTER_B_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: PRINTER_B_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(PRINTER_B_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: PRINTER_B_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PRINTER_B_IntClock_MOD_SRC |= modeBitMask & (uint8)PRINTER_B_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: PRINTER_B_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PRINTER_B_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PRINTER_B_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: PRINTER_B_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PRINTER_B_IntClock_MOD_SRC & (uint8)(PRINTER_B_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: PRINTER_B_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PRINTER_B_IntClock_GetDividerRegister();
    uint8 oldSrc = PRINTER_B_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PRINTER_B_IntClock_MOD_SRC |= CYCLK_SSS;
        PRINTER_B_IntClock_MOD_SRC =
            (PRINTER_B_IntClock_MOD_SRC & (uint8)(~PRINTER_B_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PRINTER_B_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PRINTER_B_IntClock_MOD_SRC & PRINTER_B_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PRINTER_B_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PRINTER_B_IntClock_PHASE = clkPhase & PRINTER_B_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PRINTER_B_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PRINTER_B_IntClock_PHASE & PRINTER_B_IntClock_PHASE_MASK;
}
